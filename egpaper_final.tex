...
To perform inference for a new input $x^{*}$ given a VI approximation $q(w)$ there is a need to sample the weights from the approximation for every inference run performed to extract $p\left(y^{*} \mid x^{*}, w\right)$.using Flipout \cite{Tran2019} introduced in \textit{Wen et al.}. This method aims to uncorrelated the gradients between every sample. This work achieved by making some assumptions that are 
\begin{enumerate}
